...
begin
  ...
  for var j:=StaticObjectsCount to Objects.Count-1 do
    // При столкновении пуля и объект исчезают
...
begin
  Window.Title := 'Modern Strike';
  Window.IsFixedSize := True;
  ClearWindow(clgray);
  LockDrawingObjects;
  EndOfGame := True;
  InfoString := new RectangleABC(0,0,Window.Width,38,Color.greenyellow);
  InfoString.Bordered := False;
  InfoString.FontColor := clWhite;
  InfoString.TextScale := 0.9;
  
  var zz := 100;
  NewGame := new RoundRectABC(zz,200,400,200,30,Color.yellow);
